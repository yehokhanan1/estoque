...
def adicionarPro():
    ...
    def adicionar():
        name1 = name.get().replace(" ", "").lower() + '.txt'
        if amount.get().isdigit():
            with open(name1, 'w') as digit:
                digit.write(amount.get())
            with open('produtos.txt', 'a') as digit:
                digit.write(name.get() + '\n')
            ...
            fail = Label(janela1, text="ERRO: Tente novamente", background='red',foreground='#dde')
...
def verificarPro():
    ...
    def verification():
        verificar1 = verificar.get().replace(" ", "").lower() + '.txt'
        if os.path.exists(verificar1):
            with open(verificar1, 'r') as arquivo1:
